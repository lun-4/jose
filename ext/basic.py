...
class Basic(Cog):
    ...
    async def stats(self, ctx):
        ...
        gay_chans = sum(
            1 for c in self.bot.get_all_channels() if 'gay' in c.name.lower())
    ...
    async def about(self, ctx):
        ...
        em.add_field(
            name='About',
            value='José is a generic-purpose '
            'bot (with some complicated features)')

        appinfo = await self.bot.application_info()
        owner = appinfo.owner
        em.add_field(
            name='Owner', value=f'{owner.mention}, {owner}, (`{owner.id}`)')
    ...
    async def feedback_reply(self, ctx, channel_id: int, *, message: str):
        ...
        embed = discord.Embed(
            colour=discord.Color.magenta(), description=message)
    ...
    async def clist(self, ctx, cog_name: str):
        ...
        """
        matched = [
            cmd.name for cmd in self.bot.commands if cmd.cog_name == cog_name
        ]
