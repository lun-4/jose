...
class GuildLog(Cog):
    ...
    def guild_embed(self, em, guild):
        ...
        em.add_field(name='guild id',
                     value=guild.id)

        em.add_field(name='guild name',
                     value=guild.name)
        em.add_field(name='guild owner',
                     value=guild.owner)
        em.add_field(name='guild region',
                     value=str(guild.region))
        em.add_field(name='guild member count',
                     value=guild.member_count)
        em.add_field(name='guild large?',
                     value=guild.large)
        em.add_field(name='guild <- shard id',
                     value=guild.shard_id)

    async def on_guild_join(self, guild):
        if not self.bot.is_ready():
            return

        em = discord.Embed(title='Guild join',
